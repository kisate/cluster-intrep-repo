...
def process_domain(mystery_num, output_dir="charts"):
    ...
    repr_file = f"multilayer_representations_avg/multilayer_10k/mystery_{mystery_num}/mean_reprs_mystery_{mystery_num}_multi_layer.json"
    ...
    for layer in LAYERS:
        ...
        mean_domain = avg_means[layer]["mean_domain"]
        action_own_sims = []
        pred_own_sims = []
        ...
        for step in steps:
            ...
            for phrase in unique_phrases:
                ...
                base_avg_vector = np.array(_avg_reprs[phrase]) - np.array(mean_domain)
                ...
                if is_action:
                    ...
                    for other in other_actions:
                        other_avg = np.array(_avg_reprs[other]) - np.array(mean_domain)
...
if __name__ == "__main__":
    for mystery_num in range(1, 16):
        out_dir = os.path.join("charts", "10k_avg", f"mystery_{mystery_num}")
