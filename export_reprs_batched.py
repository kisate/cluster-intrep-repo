...
# Domain-specific phrases definitions
# These map the original actions/predicates to domain-specific versions
DOMAIN_PHRASES = {
    "mystery_1": {
        "actions": {
            "attack": "attack",
...
def extract_phrase_representations_batched(ids, dataset, positions_dataset, hidden_states, layer, phrases, steps, cont_size):
...
def plot_phrases_comparison(reprs, action_phrases, predicate_phrases, clean_reprs, mystery_domain_means, clean_domain_means, output_dir, domain_name):
    ...
    clean_domain_phrases = get_domain_phrases("mystery_2")
...
def get_domain_phrases(domain_name):
    """Get domain-specific phrases from the DOMAIN_PHRASES dictionary"""
    if domain_name in DOMAIN_PHRASES:
        return DOMAIN_PHRASES[domain_name]
    else:
        print(f"Warning: No phrases defined for domain {domain_name}")
        return {
            "actions": {},
            "predicates": {}
        }

def save_representations(reprs, domain_name, output_dir):
    """Save representations to a JSON file"""
    # Calculate mean representations
    mean_reprs = {k: v.tolist() for k, v in reprs.items() if v is not None}
    
    valid_reprs = [v for v in reprs.values() if v is not None]
    if not valid_reprs:
        print(f"No valid representations to save for domain {domain_name}")
        return
        
    mean_domain = np.stack(valid_reprs).mean(0).tolist()
    
    # Get domain-specific actions and predicates
    domain_phrases = get_domain_phrases(domain_name)
    action_words = list(domain_phrases["actions"].values())
    predicate_words = list(domain_phrases["predicates"].values())
    
    # Split into actions and predicates
    valid_actions = [reprs[a] for a in action_words if a in reprs and reprs[a] is not None]
    valid_predicates = [reprs[p] for p in predicate_words if p in reprs and reprs[p] is not None]
    
    mean_actions = np.stack(valid_actions).mean(0).tolist() if valid_actions else []
    mean_predicates = np.stack(valid_predicates).mean(0).tolist() if valid_predicates else []
    ...
    os.makedirs(output_dir, exist_ok=True)
    filename = f"mean_reprs_{domain_name}_10k.json"
...
def plot_cross_domain_similarity_heatmap(mystery_reprs, clean_phrases, mystery_phrases, clean_mean_reprs, clean_domain_means, mystery_domain_means, domain_name, output_dir):
    ...
    fig.update_layout(
        ...
        height=max(600, len(clean_phrases) * 50)
    )
...
def process_mystery_domain(mystery_num, mean_reprs_clean, mean_clean_actions, mean_clean_predicates, output_dir):
    ...
    clean_domain_phrases = get_domain_phrases("mystery_2")
    mystery_domain_phrases = get_domain_phrases(f"mystery_{mystery_num}")
    ...
    steps_mystery = list(range(1000, 10000, 200))
    
    # Collect hidden states for mystery domain
    hidden_states_mystery = collect_hidden_states(correct_ids[1], datasets[1], [LAYER])
    ...
    mean_mystery_domain = np.stack([mean_reprs_mystery[p] for p in mystery_phrases], axis=0).mean(0)
    mean_mystery_actions = np.stack([mean_reprs_mystery[p] for p in mystery_action_phrases], axis=0).mean(0)
    mean_mystery_predicates = np.stack([mean_reprs_mystery[p] for p in mystery_predicate_phrases], axis=0).mean(0)
    
    clean_domain_means = {
        k: mean_clean_actions if k in clean_action_phrases else mean_clean_predicates
        for k in clean_phrases
    }
    
    mystery_domain_means = {
        k: mean_mystery_actions if k in mystery_action_phrases else mean_mystery_predicates
...
def main():
    ...
    output_dir = "./mystery_representations_greedy"
    os.makedirs(output_dir, exist_ok=True)
    ...
    try:
        ...
        hidden_states_clean = collect_hidden_states(correct_ids[0], datasets[0], [LAYER])
        ...
        mean_clean_actions = np.stack([clean_reprs[p] for p in clean_action_phrases], axis=0).mean(0)
        mean_clean_predicates = np.stack([clean_reprs[p] for p in clean_predicate_phrases], axis=0).mean(0)
        
        # Plot PCA for clean domain
        plot_pca(clean_reprs, clean_phrases, "Clean Domain (Mystery 2)", output_dir)
