...
class StepProbeDataset(Dataset):
    def __init__(self, items, n_layer, n_prev_tokens, shift_tokens, hidden_states, n_blocks, batch_size):
...
def train_func(config: dict):
    ...
    def _train_func(config: dict):
        ...
        train_items, n_layer, n_prev_tokens, n_shift_tokens, hidden_states, n_blocks, batch_size)

        test_dataset = StepProbeDataset(test_items, n_layer, n_prev_tokens, n_shift_tokens, hidden_states, n_blocks, batch_size)
        ...
        acc_meter = torchmetrics.Accuracy(task="multiclass", num_classes=n_blocks).to("cuda")
        f1_meter = torchmetrics.F1Score(task="multiclass", num_classes=n_blocks).to("cuda")
        loss_meter = torchmetrics.aggregation.MeanMetric().to("cuda")
        val_loss_meter = torchmetrics.aggregation.MeanMetric().to("cuda")
        ...
        for epoch in range(n_epochs):
            ...
            for batch in train_loader.iter_batches():
                ...
                optimizer.zero_grad()
                input = batch["input"].to(probe.device)
